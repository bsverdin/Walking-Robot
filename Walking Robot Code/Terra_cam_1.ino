...
<html>
  ...
  <body>
    ...
    <body>
          <h4>Distance (Meters): <span id="Distance">%DISTANCE%</span></h4>
        </section>
   <script>
   function toggleCheckbox(x) {
    ...
    setInterval(function() {// Call the update function every second
    var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function(){
      if(this.readyState == 4 && this.status == 200){
        document.getElementByld("Distance").innerHTML = this.responseText;
      }
    };
...
String readUltrasonicDistance() {
  digitalWrite(triggerPin, HIGH);
  delayMicroseconds(10);
  digitalWrite(triggerPin, LOW);
  duration = pulseIn(echoPin, HIGH);
  distance = duration*0.00016;
...
String giveDistance(const String& var){
  if(var == "DISTANCE"){
...
void setup() {
  ...
  server.on("/distance", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(200, "text/plain", readUltrasonicDistance());
  });
  server.begin();
}
